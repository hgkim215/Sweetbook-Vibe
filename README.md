...
| Lv2 | 성장기록 선택 후 성장기록집 주문 생성, 주문 목록/상세, 허용된 상태 전이 관리 |
...
Gemini 경로가 정상 동작하면 `챕터 제안`과 `AI 예시 기록 생성` 응답의 `source`가 `gemini`로 내려옵니다. Gemini API 키가 없거나 호출에 실패하면 앱은 자동으로 `source: "mock"`으로 전환됩니다.
...
2. 필요하면 `AI 예시 기록 생성`을 눌러 테스트용 성장기록을 1개 추가합니다.
3. 왼쪽 목록에서 성장기록을 선택합니다.
4. 성장기록을 새로 추가하거나 기존 기록을 수정합니다.
5. 책에 포함할 성장기록을 체크합니다.
6. `챕터 제안`을 눌러 성장기록집 구성을 만듭니다.
7. 책 제목, 작성자명, 요청사항을 확인하고 `주문 생성`을 누릅니다.
8. 주문 목록에서 생성된 주문을 선택합니다.
9. 주문 상세에서 `처리 시작`을 눌러 `pending -> processing`으로 변경하고, `완료 처리`를 눌러 `processing -> completed`로 변경합니다. 취소는 `pending` 또는 `processing` 상태에서만 가능합니다.
10. `JSON 다운로드`를 눌러 주문 데이터 익스포트를 확인합니다.
...
| POST | `/api/assist/sample-record` | AI 또는 Mock 예시 성장기록 1개 생성 |
| POST | `/api/assist/chapter-suggestions` | 챕터 구성 제안 |
| GET | `/api/orders` | 주문 목록 |
| POST | `/api/orders` | 주문 생성 |
| PATCH | `/api/orders/:id/status` | 주문 상태 변경. 허용 흐름: `pending -> processing -> completed`, `pending/processing -> cancelled` |
...
| Gemini API 선택 옵션 | `.env`에 키가 있을 때 SDK와 구조화 출력으로 성장기록 기반 챕터 제안과 예시 성장기록 생성을 수행하도록 설계 |
| Mock 보조 정리자 | API 키가 없어도 동일한 사용자 플로우를 검증할 수 있도록 챕터 제안과 예시 기록 생성 fallback 구현 |
