...
- 성장기록 목록 행에 `수정`/`삭제` 버튼을 추가해, 작성한 기록을 어디서 고치고 지우는지 바로 보이게 했다. 기존 편집 패널의 `수정 저장`/`삭제` 흐름은 유지하고, 삭제는 확인창을 거치도록 했다. fast/full 검증 통과 후 `origin/dev`, `origin/main`에 반영했다.
