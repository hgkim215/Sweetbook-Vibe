# 품질 점수표

상태: Lv1-Lv3 구현 완료 후 Lv2 주문 상태 전이 비즈니스 로직까지 강화 완료.

점수는 1-5점 척도를 사용한다.

| 점수 | 의미 |
|---|---|
| 5 | 제출물의 강점으로 보일 수준 |
| 4 | 제출 가능하며 작은 개선 여지만 있음 |
| 3 | 동작은 하지만 평가 리스크가 있음 |
| 2 | 구현 또는 설명이 불완전함 |
| 1 | 제출 전 반드시 수정해야 함 |

## 현재 점수

| 항목 | 점수 | 메모 |
|---|---:|---|
| Product Fit | 4 | 성장기록집 아이디어가 최종 확정되었고, 콘텐츠 본체와 책 주문 부가 기능의 연결이 명확하다. |
| Lv1 Completeness | 5 | 성장기록 CRUD, 더미 데이터, 브라우저 첫 화면, API 테스트가 모두 통과했다. |
| Lv2 Business Logic | 5 | 성장기록집 주문 생성, 조회, 상태 전이가 동작하며, 서버가 잘못된 되돌리기와 종료 상태 변경을 거부한다. |
| Lv3 Export Quality | 5 | 주문 정보, 챕터, 포함 기록, 메타데이터 JSON 익스포트가 수동 검증됐다. |
| UX Clarity | 5 | 작업 화면이 성장기록 목록, 편집, 주문 패널, 주문 상세 흐름으로 정리됐고 데스크톱/모바일 캡처와 브라우저 DOM 플로우를 통과했다. |
| 아키텍처 | 4 | React/Vite, Express, SQLite, Docker Compose 구조와 REST API 구현이 완료됐다. |
| Test Coverage | 5 | 핵심 API 테스트, Docker smoke test, 브라우저 DOM 플로우, UI 개편 후 fast/full 검증을 통과했다. |
| Docker Reliability | 5 | UI 개편 후 별도 포트에서 Docker Compose 전체 검증과 Lv1-Lv3 smoke test, 볼륨 제거가 통과했다. |
| README Quality | 5 | 실행 방법, 포트 변경, 더미 데이터, 레벨, 테스트 플로우, 주문 상태 전이 기준을 포함했다. |

## 반복 개선 규칙

각 구현 단위가 완료될 때마다 아래 순서를 따른다.

1. 모든 항목을 다시 채점한다.
2. 가장 낮은 점수의 항목을 고른다.
3. 작은 개선 작업 1개로 나눈다.
4. 구현, 검증, 커밋, `dev` 푸시를 진행한다.
5. 결과를 `docs/reliability/verification-log.md`에 기록한다.
