...
- Gemini API 키는 로컬 `.env`의 `GEMINI_API_KEY`에만 저장한다.
- `GOOGLE_API_KEY`도 fallback으로 지원하지만, 프로젝트 문서에서는 `GEMINI_API_KEY`를 표준으로 사용한다.
- Gemini 모델명은 비밀값이 아니지만, 실제 키와 같은 줄에 기록하지 않는다.
...
- `GEMINI_MODEL=gemini-2.5-flash` 같은 모델명 설정

## Gemini 키 검증 절차

1. `.env.example`을 복사해 `.env`를 만든다.
2. `.env`에만 `GEMINI_API_KEY`를 넣는다.
3. `git status --short`에서 `.env`가 보이지 않는지 확인한다.
4. 서버를 실행한 뒤 `/api/assist/chapter-suggestions` 응답이 `source: "gemini"`인지 확인한다.
...
- README, 문서, 커밋 메시지에 실제 Gemini 키가 없다.
- secret scan은 OpenAI `sk-` 형식과 Gemini `AIza` 형식을 모두 탐지한다.
